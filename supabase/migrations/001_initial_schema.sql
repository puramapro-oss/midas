-- MIDAS — Schema PostgreSQL complet
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  risk_per_trade DECIMAL DEFAULT 1.0 CHECK (risk_per_trade >= 0.5 AND risk_per_trade <= 2.0),
...
BEGIN
  ...
  VALUES (NEW.id, NEW.raw_user_meta_data->>'full_name', 'MIDAS-' || upper(substr(md5(random()::text), 1, 5)));
  RETURN NEW;
END;
...
CREATE TABLE IF NOT EXISTS public.bots (
  ...
  name TEXT NOT NULL,
  strategy TEXT NOT NULL CHECK (strategy IN ('dca', 'grid', 'momentum', 'mean_reversion', 'scalping', 'arbitrage', 'swing', 'smart_entry', 'auto')),
  pair TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.trades (
  ...
  type TEXT DEFAULT 'market' CHECK (type IN ('market', 'limit', 'stop_market', 'stop_limit')),
  ...
  close_reason TEXT CHECK (close_reason IN ('take_profit', 'stop_loss', 'trailing_stop', 'manual', 'circuit_breaker', 'crash_protection', 'shield_limit', 'error')),
...
CREATE POLICY "Users own exchanges" ON public.exchange_connections FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users own bots" ON public.bots FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users own trades" ON public.trades FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users own chat" ON public.chat_messages FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users own performance" ON public.daily_performance FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users own audit" ON public.audit_logs FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Signals public read" ON public.signals FOR SELECT USING (true);
CREATE POLICY "Market cache public read" ON public.market_cache FOR SELECT USING (true);
...
-- ══════ SEED SUPER ADMIN ══════
-- This runs after user signs up via auth; the trigger creates the profile.
-- To manually seed: UPDATE public.profiles SET plan='ultra', risk_profile='moderate' WHERE id = (SELECT id FROM auth.users WHERE email='matiss.frasne@gmail.com');
